...
Ждёмс
> [!IMPORTANT]
> Если же написало youtube.com `Works without dpi` или подобное то ютуб работает без дпи, он всё таки не нужен, пропускаем этот шаг.
...
---
Вместо `<HOSTLIST>` <das> вставляем путь до нашего хостлиста `--hostlist=/opt/zapret/lists/googlevideo.txt`
...
И запускаем их, выдыхая ВСЕ проблемы и запреты РКН
