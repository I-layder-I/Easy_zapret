...
> [!Caution]
> Также (в случае ввода просто `cd` - для быстрого перехода в домашний каталог), то есть заместо `layder` в данном случае моё имя, пользователя будет `~` и чтобы вернуться в домашнюю папку пишем
> ```
> cd /home/layder
> ```
